# Team Carpinchos - Official Website

This repository contains the source code for the official website of Team Carpinchos, the Student Cluster Competition team from the Universidad Nacional de Córdoba (UNC), Argentina. The site is built using Jekyll and hosted on GitHub Pages.

## Website Structure

*   **`_config.yml`**: Main Jekyll configuration file (site title, theme, plugins, navigation).
*   **`index.md`**: Homepage content.
*   **`_posts/`**: Directory containing news articles as Markdown files. File names must follow the `YYYY-MM-DD-title.md` format.
*   **`_data/`**: Directory containing structured data files.
    *   `events.yml`: Data for the events list.
    *   `members.yml`: Data for the team roster.
*   **`assets/`**: Directory for static files like images and CSS (if customizing).
    *   `images/`: Contains profile pictures and other site images.
*   **`_layouts/`, `_includes/`, `_sass/`**: (Optional) Directories for customizing the theme's layout, reusable components, and styling.
...
2.  Name the file `YYYY-MM-DD-your-post-title.md`.
3.  Add the following front matter at the top:
    ```yaml
    ---
    layout: post
    title: "Your Post Title"
    date: YYYY-MM-DD HH:MM:SS -ZZZZ # e.g., 2025-04-10 09:00:00 -0300
    categories: update # Optional category
    ---
    ```
...
1.  Edit the `_data/events.yml` file.
2.  Add or modify entries following the existing format:
    ```yaml
    - year: YYYY
      name: Event Name
      location: City, Country
      details: Brief description of participation or results.
    ```

### Updating the Roster

Edit the `_data/members.yml` file.
...
2.  Run `bundle config set path '.bundle/vendor' --local` to store dependencies in a subdirectory.
...
Changes pushed to the `main` branch of the GitHub repository will automatically update the live site on GitHub Pages.

---

Fragmentos del README del tema:
